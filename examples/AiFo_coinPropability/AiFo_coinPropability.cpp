...
void AiFo_coinPropability_Loop();
void AiFo_coinPropability_SetupWeights(Net* predictorNet, Net* backwardsPredictor);
...
void AiFo_coinPropability_Loop()
{
	Display display(sf::Vector2u(1900,1300),"X-OR Example");

	
	BackpropNet net;
	Net chancePredictorNet;
	chancePredictorNet.setDimensions(1, 1, 3, 8);
	chancePredictorNet.setActivation(Activation::linear);
	chancePredictorNet.setHardware(Hardware::cpu);
	chancePredictorNet.enableBias(false);
	chancePredictorNet.build();
	
	//MultiSignalVector trainigsSet(4, 2);
	//MultiSignalVector resultSet(4, 1);

	net.setDimensions(2, 0, 0, 3);
	...
	AiFo_coinPropability_SetupWeights(&chancePredictorNet , &net);
	MultiSignalVector coinFlips(3, 2);
	coinFlips[0] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[1] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[2] = SignalVector(vector<float>{ 1, 0 });

	// Take a blue coin
	/*MultiSignalVector coinFlips(10, 2);
	coinFlips[0] = SignalVector(vector<float>{ 0, 1 }); // Head Tail
	coinFlips[1] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[2] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[3] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[4] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[5] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[6] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[7] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[8] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[9] = SignalVector(vector<float>{ 1, 0 });*/

	// Take a green coin
	/*MultiSignalVector coinFlips(10, 2);
	coinFlips[0] = SignalVector(vector<float>{ 0, 1 }); // Head Tail
	coinFlips[1] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[2] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[3] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[4] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[5] = SignalVector(vector<float>{ 1, 0 });
	coinFlips[6] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[7] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[8] = SignalVector(vector<float>{ 0, 1 });
	coinFlips[9] = SignalVector(vector<float>{ 0, 1 });*/
	...
	NetModel netModel1(&chancePredictorNet);
	...
	netModel1.setNeuronSize(50);
	netModel1.setConnectionWidth(20);
	netModel1.setNeuronSpacing(sf::Vector2f(100, 50));
	//netModel1.setDrawDebug(true);
	display.addDrawable(&netModel1);

	NetModel netModel2(&net);
	netModel2.setStreamIndex(0);
	netModel2.setNeuronSize(neuronSize);
	netModel2.setPos(sf::Vector2f(1000, 100));
	netModel2.setNeuronSpacing(spacing);
	netModel2.setVisualConfiguration(visualConfig);
	netModel2.setNeuronSize(50);
	netModel2.setConnectionWidth(20);
	netModel2.setNeuronSpacing(sf::Vector2f(100,50));
	...
	while (display.isOpen())
	{
		...
		{
			++iteration;
			for (size_t k = 0; k < coinFlips.size(); ++k)
			{

				trainigTimer.unpause();
				//net.setInputVector(trainigsSet);
				chancePredictorNet.setInput(0, 1);


				net.setInput(0, 1);
				net.setInput(1, 0);
				net.calculate();
				chancePredictorNet.calculate();
				SignalVector flipChange = chancePredictorNet.getOutputVector();
				float headChance = flipChange[0];
				float tailChance = flipChange[1];

				float redHeadChance = flipChange[2];
				float blueHeadChance = flipChange[4];
				float greenHeadChance = flipChange[6];

				float redTailChance = flipChange[3];
...
void AiFo_coinPropability_SetupWeights(Net* predictorNet, Net* backwardsPredictor)
{
	float chance_red_tail = 0.5;
	float chance_blue_tail = 0.3;
	float chance_green_tail = 0.9;

	float chance_red = 1.f / 3.f;
	float chance_blue = 1.f / 3.f;
	float chance_green = 1.f / 3.f;

	// Setze alle Gewichte auf 0
	vector<float> weights(predictorNet->getWeightSize(), 0);
	predictorNet->setWeight(weights);

	// Setze die gewichte für die chance, dass eine farbe ausgewählt wurde
	predictorNet->setWeight(0, 0, 0, chance_red);
	predictorNet->setWeight(0, 1, 0, chance_blue);
	predictorNet->setWeight(0, 2, 0, chance_green);

	
	predictorNet->setWeight(1, 0, 0, 1.f - chance_red_tail); // Gewicht rot gewählt - zum Ausgang 0 (head)
	predictorNet->setWeight(1, 1, 0, chance_red_tail);       // Gewicht rot gewählt - zum Ausgang 0 (head)
	predictorNet->setWeight(1, 2, 0, 1.f - chance_red_tail);
	predictorNet->setWeight(1, 3, 0, chance_red_tail);

	predictorNet->setWeight(1, 0, 1, 1.f - chance_blue_tail);
	predictorNet->setWeight(1, 1, 1, chance_blue_tail);
	predictorNet->setWeight(1, 4, 1, 1.f - chance_blue_tail);
	predictorNet->setWeight(1, 5, 1, chance_blue_tail);

	predictorNet->setWeight(1, 0, 2, 1.f - chance_green_tail);
	predictorNet->setWeight(1, 1, 2, chance_green_tail);
	predictorNet->setWeight(1, 6, 2, 1.f - chance_green_tail);
	predictorNet->setWeight(1, 7, 2, chance_green_tail);

	/*predictorNet->setWeight(1, 0, 0, 0);
	predictorNet->setWeight(1, 1, 0, 0);

	predictorNet->setWeight(1, 0, 1, 0);
	predictorNet->setWeight(1, 1, 1, 0);

	predictorNet->setWeight(1, 0, 2, 0);
	predictorNet->setWeight(1, 1, 2, 0);*/

	
	cout << "weights\n";
	const float* w = predictorNet->getWeight();
	for (size_t i = 0; i < predictorNet->getWeightSize(); ++i)
	{
		cout << w[i] << "  ";
	}
	cout <<  "\n";
	...
	// Neuron 1 = red coin
	backwardsPredictor->setWeight(0, 0, 0, chance_red_tail); // input 1 = tail
	backwardsPredictor->setWeight(0, 0, 1, 1 - chance_red_tail); // input 2 = head

	// Neuron 2 = blue coin
	backwardsPredictor->setWeight(0, 1, 0, chance_blue_tail); // input 1 = tail
	backwardsPredictor->setWeight(0, 1, 1, 1 - chance_blue_tail); // input 2 = head

	// Neuron 3 = green coin
	backwardsPredictor->setWeight(0, 2, 0, chance_green_tail); // input 1 = tail
	backwardsPredictor->setWeight(0, 2, 1, 1 - chance_green_tail); // input 2 = head
